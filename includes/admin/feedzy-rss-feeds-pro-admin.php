...
class Feedzy_Rss_Feeds_Pro_Admin {
	...
	private function delete_old_posts( $job ) {
		...
		$delete_media = get_post_meta( $job->ID, 'import_feed_delete_media', true );
		...
		$delete_media     = $delete_media ? 'yes' === $delete_media : false;
	...
	public function magic_tags_post_excerpt( $default ) {
		if ( $this->feedzy_is_agency() ) {
			$default['translated_title']       = __( 'Translated Title', 'feedzy-rss-feeds-pro' );
			$default['translated_content']     = __( 'Translated Content', 'feedzy-rss-feeds-pro' );
			$default['translated_description'] = __( 'Translated Description', 'feedzy-rss-feeds-pro' );
		} else {
			$default['translated_title:disabled']       = '🚫 ' . __( 'Translated Title', 'feedzy-rss-feeds-pro' );
			$default['translated_content:disabled']     = '🚫 ' . __( 'Translated Content', 'feedzy-rss-feeds-pro' );
			$default['translated_description:disabled'] = '🚫 ' . __( 'Translated Description', 'feedzy-rss-feeds-pro' );
		}
	...
	public function magic_tags_content( $default ) {
		if ( $this->feedzy_is_business() ) {
			$default['item_full_content'] = __( 'Item Full Content', 'feedzy-rss-feeds-pro' );
		} else {
			$default['item_full_content:disabled'] = '🚫 ' . __( 'Item Full Content', 'feedzy-rss-feeds-pro' );
		}
	...
	public function delete_attached_thumbnail( $post_id = 0, $post_type = 'post' ) {
		...
		if ( empty( $media_ids ) ) {
			...
			do_action( 'themeisle_log_event', FEEDZY_NAME, sprintf( 'Deleted attached featured image ID: %d, Post: %d', $thumbnail_id, $post_id ), 'info', __FILE__, __LINE__ );
		}
	}
