...
class Feedzy_Rss_Feeds_Pro_Amazon_Product_Advertising implements Feedzy_Rss_Feeds_Pro_Services_Interface {
	...
	public function call_api( $access_key, $secret_key, $partner_tag, $additional = array() ) {
		...
		if ( ! $this->feedzy_is_business() ) {
			$this->errors[] = esc_html__( 'Invalid license plan', 'feedzy-rss-feeds-pro' ) . sprintf( ', <a href="%1$s" target="_blank">%2$s</a>.', tsdk_translate_link( tsdk_utmify( FEEDZY_UPSELL_LINK, 'amazon-product' ) ), esc_html__( 'Please upgrade plan', 'feedzy-rss-feeds-pro' ) );
			return $this;
		}
	...
	public function get_service_name_proper() {
		return __( 'Amazon Product Advertising', 'feedzy-rss-feeds-pro' );
	}
	...
	public function get_title() {
		return __( 'Amazon', 'feedzy-rss-feeds-pro' );
	}
	...
	public function error() {
		...
		$errors = reset( $errors );
		return wp_sprintf(
			// translators: %s: Error message from Amazon Product Advertising API.
			__( '[Amazon Product Advertising API] %s', 'feedzy-rss-feeds-pro' ),
			$errors
		);
	}
