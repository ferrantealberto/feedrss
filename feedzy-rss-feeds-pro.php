...
function run_feedzy_rss_feeds_pro() {
	...
	define( 'FEEDZY_PRO_VERSION', '3.0.0' );
	...
	add_filter(
		'feedzy_rss_feeds_float_widget_metadata', function ( $metadata ) {
		if ( empty( $metadata ) ) {
			return $metadata;
		}
	...
	add_filter(
		...
		function ( $message ) {
			return str_replace( '<a href="%s">', '<a href="' . admin_url( 'admin.php?page=feedzy-settings' ) . '">', $message );
		}
	);
